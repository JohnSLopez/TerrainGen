...
private:
	...
	void drawFrame()
	{
		...
		//Wait for semaphores at color attachment output stage
		...
		//Semaphore to signal when command buffer is finished executing
	...
	{
		//get list of available extensions for device
		...
		//remove extension from required extension list if found in available extensions
		for (const auto& extension : availableExtensions)
		{
			requiredExtensions.erase(extension.extensionName);
		}

		//If list is empty then all required extensions were found
	...
	struct QueueFamilyIndices
	{
		...
		bool isComplete()
		{
			//Return true if graphics and surface queues found
	...
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device)
	{
		...
		//Get queue families from selected physical device
		...
		{
			//Check for surface queue support
			...
			//Check if suitable queue families found
	...
	void createGraphicsPipeline()
	{
		...
		{
			throw std::runtime_error("faled to create graphics pipeline");
		}

		//Shader modules can be destroyed after being loaded in pipeline
	...
	void createRenderPass()
	{
		...
		colorAttachmentRef.attachment = 0; //Layout location will be 0
		...
		//dependency subpass, implicit subpass before render pass
		dependency.srcSubpass = VK_SUBPASS_EXTERNAL;
		//dependent subpass, 0 = my subpass
		dependency.dstSubpass = 0;
		//Wait for color attachment output stage
		dependency.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
		dependency.srcAccessMask = 0;
		//Wait in color attachment stage for writing operation
	...
	static std::vector<char> readFile(const std::string& filename)
	{
		...
		//Get size of shader in bytes and create array to store 
		size_t fileSize = (size_t)file.tellg();
		std::vector<char> buffer(fileSize);

		//Go back to start of stream and store shader in buffer
		file.seekg(0);
	...
	void createTextureImage()
	{
		...
		stbi_uc* pixels = stbi_load("textures/borz3.jpg", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	void createIndexBuffer()
	{
		...
		//Create transfer source buffer
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingBufferMemory);

		//Map and copy indices data to memory
		void* data;
		vkMapMemory(device, stagingBufferMemory, 0, bufferSize, 0, &data);
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
	{
		...
		//Viewport and Scissor are dynamic so must be set here
		VkViewport viewport{};
		viewport.x = 0.0f;
		viewport.y = 0.0f;
	...
	void createSyncObjects()
	{
		...
		//Start in signalled state so program doesn't stall on first frame
