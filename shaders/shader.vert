...
float HashPosition(vec2 pos) {
	return pseudo(pos * vec2(ubo._Seed, ubo._Seed + 4));
}
...
vec3 fbm(vec2 pos) {
	float lacunarity = ubo._Lacunarity;
	float amplitude = ubo._InitialAmplitude;
	...
	float angle_variance = mix(ubo._AngularVariance.x, ubo._AngularVariance.y, HashPosition(vec2(ubo._Seed, 827)));
	float theta = (ubo._NoiseRotation + angle_variance) * PI / 180.0;
	...
	for(int i = 0; i < int(ubo._Octaves); ++i) {
		...
		amplitude *= ubo._AmplitudeDecay;
				
		// generate random angle variance if applicable
		angle_variance = mix(ubo._AngularVariance.x, ubo._AngularVariance.y, HashPosition(vec2(i * 419, ubo._Seed)));
		theta = (ubo._NoiseRotation + angle_variance) * PI / 180.0;
		...
		float freq_variance = mix(ubo._FrequencyVarianceLowerBound, ubo._FrequencyVarianceUpperBound, HashPosition(vec2(i * 422, ubo._Seed)));
